...
fn deth_empty() {
    macro_rules! deth_empty {
        ($elem:ty) => {
            ...
            assert_eq!(a.factorizeh().unwrap().sign_ln_deth(), (One::one(), Zero::zero()));
            assert_eq!(a.factorizeh().unwrap().deth_into(), One::one());
            assert_eq!(a.factorizeh().unwrap().sign_ln_deth_into(), (One::one(), Zero::zero()));
            assert_eq!(a.deth().unwrap(), One::one());
            assert_eq!(a.sign_ln_deth().unwrap(), (One::one(), Zero::zero()));
            assert_eq!(a.clone().deth_into().unwrap(), One::one());
            assert_eq!(a.sign_ln_deth_into().unwrap(), (One::one(), Zero::zero()));
        }
    }
    deth_empty!(f64);
    deth_empty!(f32);
...
fn deth_zero() {
    macro_rules! deth_zero {
        ($elem:ty) => {
            ...
            assert_eq!(a.sign_ln_deth().unwrap(), (Zero::zero(), Float::neg_infinity()));
            assert_eq!(a.clone().deth_into().unwrap(), Zero::zero());
            assert_eq!(a.sign_ln_deth_into().unwrap(), (Zero::zero(), Float::neg_infinity()));
        }
    }
    deth_zero!(f64);
    deth_zero!(f32);
...
fn deth_zero_nonsquare() {
    macro_rules! deth_zero_nonsquare {
        ($elem:ty, $shape:expr) => {
            ...
            assert!(a.sign_ln_deth().is_err());
            assert!(a.clone().deth_into().is_err());
            assert!(a.sign_ln_deth_into().is_err());
        }
    }
